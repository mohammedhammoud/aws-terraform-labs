data "aws_iam_policy_document" "ecs_fargate_e2e_cf_s3" {
...
resource "aws_iam_policy" "ecs_fargate_e2e_cf_s3" {
  for_each = local.ecs_fargate_e2e_ci_instances

  name   = "${each.value.apply_role_name}-cf-s3"
  policy = data.aws_iam_policy_document.ecs_fargate_e2e_cf_s3[each.key].json
}

resource "aws_iam_role_policy_attachment" "ecs_fargate_e2e_cf_s3" {
  for_each = local.ecs_fargate_e2e_ci_instances

  role       = aws_iam_role.ecs_fargate_e2e_github_actions_apply[each.key].name
  policy_arn = aws_iam_policy.ecs_fargate_e2e_cf_s3[each.key].arn
}
